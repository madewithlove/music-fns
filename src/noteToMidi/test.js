...
describe('noteToMidi', () => {
  ...
  it('should convert "C4" to its correct midi number', () => {
    const note = 'C4';
    expect(noteToMidi(note, { standard: 'C4' })).toEqual(60);
  });

  it('should convert "C0" to its correct midi number', () => {
    const note = 'C0';
    expect(noteToMidi(note)).toEqual(24);
  });

  it('should convert "G8" to its correct midi number', () => {
    const note = 'G8';
    expect(noteToMidi(note)).toEqual(127);
  });
  ...
  it('should convert "A#6" to its correct midi number', () => {
    const note = 'A#6';
    expect(noteToMidi(note)).toEqual(106);
  });
  ...
  it('should throw error on invalid midi note standard', () => {
    const note = 'C4';
    expect(() => noteToMidi(note, { standard: 4 })).toThrow();
  });
});
